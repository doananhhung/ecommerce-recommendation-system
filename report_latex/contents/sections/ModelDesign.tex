...
Trong kiến trúc hệ gợi ý hai giai đoạn, mô hình Recall có nhiệm vụ lọc nhanh một tập sản phẩm ứng viên từ toàn bộ danh mục sản phẩm. Đây là bước đầu tiên trong quá trình gợi ý, giúp giảm đáng kể không gian tìm kiếm trước khi chuyển sang giai đoạn Ranking. Nếu hệ thống có hàng chục nghìn hoặc hàng triệu sản phẩm, việc đưa toàn bộ sản phẩm vào mô hình Ranking sẽ tốn nhiều chi phí tính toán và không phù hợp với yêu cầu phản hồi nhanh.
\medskip

Mục tiêu của Recall không phải là sắp xếp chính xác tuyệt đối tất cả sản phẩm, mà là bảo đảm các sản phẩm có khả năng phù hợp cao với người dùng được giữ lại trong tập ứng viên. Nói cách khác, Recall cần có khả năng bao phủ tốt các sản phẩm tiềm năng. Sau đó, mô hình Ranking sẽ tiếp tục phân tích chi tiết hơn và quyết định thứ tự cuối cùng của các sản phẩm trong danh sách gợi ý.
\medskip
...
Matrix Factorization là một phương pháp phổ biến trong hệ thống gợi ý, đặc biệt phù hợp với dữ liệu tương tác người dùng-sản phẩm (user-item) có độ thưa cao. Ý tưởng chính của phương pháp này là biểu diễn mỗi người dùng và mỗi sản phẩm bằng một vector trong không gian ẩn có số chiều nhỏ hơn nhiều so với số lượng người dùng và sản phẩm ban đầu. Các vector này được học thông qua quá trình huấn luyện sao cho người dùng và sản phẩm có quan hệ phù hợp sẽ có vector gần nhau hơn.
\medskip
...
Một điểm quan trọng trong quá trình huấn luyện là cách chia tập huấn luyện và tập kiểm thử. Thay vì chia ngẫu nhiên, dự án sắp xếp dữ liệu theo \texttt{event\_time} và sử dụng cách chia theo thời gian (time-based split) với tỷ lệ 80\% cho huấn luyện và 20\% cho kiểm thử. Cách chia này phù hợp hơn với bài toán gợi ý vì mô hình trong thực tế luôn được huấn luyện trên dữ liệu quá khứ và dự đoán cho các hành vi xảy ra sau đó.
\medskip
...
\caption[Quy trình tạo dữ liệu Train-on-Recall]{Quy trình tạo dữ liệu Train-on-Recall trong \texttt{src/ranking\_model/train\_on\_recall.py}}
